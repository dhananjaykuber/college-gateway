...
namespace College_Gateway\Inc\Helpers;
...
function autoloader( $resource = '' ) { // phpcs:ignore
	...
	if ( 'inc' === $path[0] ) {
		...
		$resource_path = sprintf( '%s/inc/%s/%s.php', untrailingslashit( COLLEGE_GATEWAY_PATH ), $directory, $file_name );
...
spl_autoload_register( '\College_Gateway\Inc\Helpers\autoloader' );
